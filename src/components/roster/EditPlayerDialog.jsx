...
export default function EditPlayerDialog({ player, open, onClose, onSaved }) {
  ...
  useEffect(() => {
    if (player) {
      setForm({
        ...
        primary_position: player.primary_position || "Pitcher",
        secondary_position: player.secondary_position || "None",
        batting_hand: player.batting_hand || "Right",
        throwing_hand: player.throwing_hand || "Right",
        parent_names: player.parent_names || "",
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setSaving(true);
    await base44.entities.Player.update(player.id, { ...form, jersey_number: Number(form.jersey_number) });
    setSaving(false);
    onSaved();
  };
