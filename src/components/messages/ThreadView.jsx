...
export default function ThreadView({ thread, onBack, coachName }) {
  ...
  const handleSend = async (e) => {
    ...
    await base44.entities.Message.create({
      ...
      sender_role: "coach", sender_name: coachName || "Coach", recipient_name: thread.parent_name,
    });
    await base44.entities.Thread.update(thread.id, { last_message_body: reply, last_sender_role: "coach" });
    setReply("");
    setSending(false);
    loadMessages();
  };
        ...
        <div className="p-5 space-y-4 max-h-[400px] overflow-y-auto">
          {loading ? (
            ...
            <div key={msg.id} className={`flex ${msg.sender_role === "coach" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 ${msg.sender_role === "coach" ? "bg-red-800 text-white" : "bg-slate-100 text-slate-900"}`}>
                <p className="text-sm">{msg.body}</p>
                <p className={`text-xs mt-1 ${msg.sender_role === "coach" ? "text-red-200" : "text-slate-400"}`}>
